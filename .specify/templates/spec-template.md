...
- **FR-006**: If the feature includes signals, predictions, strategy, goals, or
...
- **FR-007**: If the feature depends on external or cached data, it MUST expose
  source status, fallback behavior, stale-data handling, and graceful failure
  behavior.
- **FR-008**: If the feature changes reportable analytics or hierarchy, PDF and
  Markdown exports MUST remain aligned and AI-readable.
- **FR-009**: The feature MUST preserve existing modules, routes, configuration,
  cache files, and local SQLite data unless a migration plan is specified.
- **FR-010**: The feature MUST avoid paid APIs unless explicit approval and
...
- **SC-005**: Strategy or recommendation outputs can be interpreted as
  decision-support context without hard buy/sell instructions.
- **SC-006**: Report/export changes are readable in both browser/PDF form and
